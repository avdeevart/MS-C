...
int ReturnSumMainDiaganal(int[,] inputArray)    //метод, возращающий решение задачи№51
{
    int summ = 0;
    int minSide = 0;
    if (inputArray.GetLength(0) < inputArray.GetLength(1))  //чтобы индекс не выходил за пределы массива
    { minSide = inputArray.GetLength(0); }                  //ограничиваем его по минимальному размеру (строки или столбца)
    else { minSide = inputArray.GetLength(1); }
